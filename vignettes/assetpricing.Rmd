...
The classic Fama-French (1993) pattern is visible: **small-cap value portfolios
have large positive CAPM alphas; large-cap growth portfolios have negative alphas**.
CAPM cannot account for the size and value premiums.
...
Both models are rejected at conventional levels by the GRS test. However, the
FF3 GRS statistic is substantially smaller than the CAPM statistic and the mean
absolute alpha is sharply reduced, confirming that SMB and HML capture a large
fraction of the size and value patterns in returns.
...
- **CAPM**: The market beta price ($\lambda_{\text{Mkt}}$) is typically small
  and statistically insignificant, while the intercept is significantly positive.
  A priced factor should have $\lambda \approx$ mean factor return and intercept
  $\approx 0$; the CAPM fails both conditions.

- **FF3**: SMB and HML command positive and significant risk prices in the
  cross-section, consistent with size and value being priced risks. The intercept
  shrinks toward zero.

- **FF4 (Carhart)**: Momentum carries a positive and significant price of risk,
  confirming that it adds cross-sectional explanatory power beyond FF3.

- **FF5**: RMW (profitability) and CMA (investment) are positive. Including
  these sometimes attenuates the HML price of risk, consistent with Fama and
  French (2015) who argue HML is partially redundant given RMW and CMA.
...
The 25 Size×BM portfolios present a severe challenge to the CAPM. Adding SMB
and HML under FF3 dramatically reduces pricing errors but does not pass the
GRS test. Momentum remains an anomaly under CAPM and FF3. The FMB regressions
confirm that SMB, HML, and momentum betas are associated with positive risk
premia in the cross-section of these portfolios.
